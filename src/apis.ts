...
const autoDowngradeForLowVersionBrowser = (configuration: FrameworkConfiguration): FrameworkConfiguration => {
  ...
  if (sandbox) {
    ...
    if (!window.Proxy) {
      ...
      // 非单实例场景下提示 不支持多实例
      if (singular === false) {
        // 因为浏览器不支持Proxy，所以设置成多实例可能会导致意外的错误
        console.warn(
    ...
    registerApplication({
      ...
      app: async () => {
        ...
        return {
          ...
          ...otherMicroAppConfigs, // bootatrap、name、unmount
        };
      },
...
export function start(opts: FrameworkConfiguration = {}) {
  /**
   * singular: 是否为单实例场景（同一时间只会渲染一个微应用）
   */
